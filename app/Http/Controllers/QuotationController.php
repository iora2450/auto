...
class QuotationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($document){
          ...
          if($data["tipo_solicitud"]=="cotizacion"){
            //correlativo 
            // codigo de clinte 
            // fcha
             $datos = DB::select('SELECT COUNT(*) AS total FROM `quotations`
                                  WHERE RIGHT(reference_no,4) = YEAR(NOW()) ');
             ...
             if($datos[0]->total ==0 ){
                ...
                $datos = DB::select("SELECT MAX(SUBSTRING_INDEX(reference_no, '-',1) )+1 AS correlativo  
                    FROM quotations WHERE RIGHT(reference_no,4) = YEAR(NOW())  ");
            ...
            $codigocliente=$lims_customer_data->code; 
            $fecha = date("dmY");

            $data['reference_no'] = $correlativo.'-'.$codigocliente.'-'.$fecha; 
        }
        //$data['reference_no'] = 'qr-' . date("Ymd") . '-'. date("his");
        $data['type_quotations'] = $data["tipo_solicitud"];
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_quotation['unit_cost'] = $lims_product_data->cost;
            $product_quotation['gain']      = ($net_unit_price[$i]-$lims_product_data->cost)*$qty[$i];
            if($net_unit_price[$i] >0){

            $product_quotation['gain_porc'] = number_format((($net_unit_price[$i]-$lims_product_data->cost)/$net_unit_price[$i])*100,2);
        ...
        if($data["tipo_solicitud"]=="remision"){

         $nuevocorrelativo =(int)$data['reference_no']+1;
        
          $message=$data['reference_no'];


         $data_actualiza["correlativo"] =$nuevocorrelativo; 
         /*4 es el correlativo de notas de remision, solo lo actualizaremos si se trata de una cotizacion tipo nota de remision*/
    ...
    public function productQuotationData($id)
    {
        ...
        foreach ($lims_product_quotation_data as $key => $product_quotation_data) {
            ...
            $product_quotation[6][$key] = $product_quotation_data->total;
             $product_quotation[7][$key] = $product_quotation_data->tittle;
              $product_quotation[8][$key] = $product_quotation_data->name_seller;
               $product_quotation[9][$key] = $product_quotation_data->cell_seller;
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($product_id as $i => $pro_id) {
            ...
            $input['unit_cost'] = $lims_product_data->cost;
            $input['gain']      = ($net_unit_price[$i]-$lims_product_data->cost)*$qty[$i];
            $input['gain_porc'] = number_format((($net_unit_price[$i]-$lims_product_data->cost)/$net_unit_price[$i])*100,2);
    ...
    public function createSale($id)
    {
        ...
        return view('quotation.create_sale',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_quotation_data','lims_product_quotation_data', 'lims_pos_setting_data','lims_documents_list'));
    }
