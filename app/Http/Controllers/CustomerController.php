...
class CustomerController extends Controller
{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('customers-add')){
            ...
            $lims_type_taxpayer_list = Type_taxpayer::all();
            $lims_customer_gyre = Gire::all();
            $lims_customer_country = Country::all();            
            $lims_customer_state = State::all();
            $lims_customer_municipalities = Municipality::with("state")->get();

            return view('customer.create', compact('lims_customer_group_all','lims_type_taxpayer_list','lims_customer_gyre','lims_customer_country','lims_customer_state','lims_customer_municipalities'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    $existingCustomer = Customer::where('phone_number', $request->phone_number)
                                ->where('is_active', 1)
                                ->first();
    
    if ($existingCustomer) {
        // Si el número ya existe, redirigir de nuevo con un mensaje de error sin perder los datos
   ...
   exit();
        ...
        try {
            // Creación de usuario si es requerido
            if (isset($lims_customer_data['user'])) {
                ...
                $lims_customer_data['nit'] = $lims_customer_data['nit'];
                $lims_customer_data['role_id'] = 5;
                $lims_customer_data['is_deleted'] = false;
                $lims_customer_data['type_taxpayer_id'] = $lims_customer_data['type_taxpayer_id'];
            ...
            // Enviar correo si se especifica el email
            ...
            return redirect()->route('customer.index')->with('create_message', $message);
    
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'An error occurred while creating the customer: ' . $e->getMessage())->withInput();
        }
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('customers-edit')){
            ...
            $lims_customer_country = Country::all();
            $lims_customer_state = State::all();            
            $lims_customer_municipality = Municipality::all();
            $lims_type_taxpayer_list = Type_taxpayer::all();
            return view('customer.edit', compact('lims_customer_data','lims_customer_group_all','lims_type_taxpayer_list', 'lims_customer_gyre', 'lims_customer_country', 'lims_customer_state','lims_customer_municipality'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
