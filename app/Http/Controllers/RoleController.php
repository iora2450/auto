...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('typedocument-index')){
            $permission = Permission::firstOrCreate(['name' => 'typedocument-index']);
            if(!$role->hasPermissionTo('typedocument-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('typedocument-index');

        if($request->has('typedocument-add')){
            $permission = Permission::firstOrCreate(['name' => 'typedocument-add']);
            if(!$role->hasPermissionTo('typedocument-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('typedocument-add');

        if($request->has('typedocument-edit')){
            $permission = Permission::firstOrCreate(['name' => 'typedocument-edit']);
            if(!$role->hasPermissionTo('typedocument-edit')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('typedocument-edit');

        if($request->has('typedocument-delete')){
            $permission = Permission::firstOrCreate(['name' => 'typedocument-delete']);
            if(!$role->hasPermissionTo('typedocument-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('typedocument-delete');
        
        if($request->has('quedanpurchase-index')){
            $permission = Permission::firstOrCreate(['name' => 'quedanpurchase-index']);
            if(!$role->hasPermissionTo('quedanpurchase-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('quedanpurchase-index');

        if($request->has('quedanpurchase-add')){
            $permission = Permission::firstOrCreate(['name' => 'quedanpurchase-add']);
            if(!$role->hasPermissionTo('quedanpurchase-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('quedanpurchase-add');
