...
<script type="text/javascript">
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                if(flag){
                    ...
                    cols += '<td>' + data[0] + '</td>';
                    cols += '<td>' + data[1] + '</td>';
                    cols += '<td><input type="number" class="form-control qty" name="qty[]" value="1" step="0.01" required/></td>';
                    ...
                    cols += '<input type="hidden" class="product-cost" name="product_cost[]" value="' + data[3] + '"/>';
	...
	function checkQuantity(qty) {
	    ...
	    if ( (qty > parseFloat(product_qty[pos])) && (action == '-') ) {
            ...
            var row_qty = parseFloat($('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty').val());
            row_qty = row_qty.substring(0, row_qty.length - 1);
            parseFloat($('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty').val(row_qty));
	    }
	    else {
	        //parseFloat($('table.order-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.qty').val(qty));
...
<?php echo $__env->make('layout.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home2/mrjbinve/public_html/sistema/resources/views/adjustment/create.blade.php ENDPATH**/ ?>