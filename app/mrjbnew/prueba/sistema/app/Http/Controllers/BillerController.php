...
class BillerController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $image = $request->image;
        $image_firma = $request->image_firma;
        if ($image) {
           ...
           // $imageName = preg_replace('/[^a-zA-Z0-9]/', '', $request['company_name']);
            ...
            $lims_biller_data['image'] = $imageName;
                ...
                if ($image_firma) {
            $ext = pathinfo($image_firma->getClientOriginalName(), PATHINFO_EXTENSION);
           // $imageName = preg_replace('/[^a-zA-Z0-9]/', '', $request['company_name']);
             $imageName = rand();
            /*Image::make($image)
                ->resize(250, null, function ($constraints) {
            ...
            $imageName = $imageName . '.' . $ext;
            $image_firma->move('public/images/biller', $imageName);
            
            $lims_biller_data['image_firma'] = $imageName;
        }
