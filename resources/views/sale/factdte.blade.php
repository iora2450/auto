...
<body> 
    ...
    <div id="receipt-data">
        ...
        <div class="dheader">
            <div class="qr" style="margin-top: -150px; padding-left: 240px;">            
...
<div class="container-fluid" id="tabla1">
    ...
    <table style='width: 100%; border: 1px solid black; border-collapse: collapse;'>
        <tr>
            <td style='width: 100%; border: 1px solid black;'><strong>CASA EXPORTADOR:</strong> {{ $lims_sale_data->casa_exportador }}</td>
        </tr>
        <tr>
            <td style='width: 100%; border: 1px solid black;'><strong>FACTURAS:</strong> {{ $lims_sale_data->facturas }}</td>
        </tr>
        <tr>
            <td style='width: 50%; border: 1px solid black;'><strong>DUCA:</strong> {{ $lims_sale_data->duca }}</td>

        </tr>
        <tr>
            <td style='width: 50%; border: 1px solid black;'><strong>ADUANA:</strong> {{ $lims_sale_data->aduana }}</td>
        </tr>
    </table>
</div>
    ...
    <tr>
        ...
        <th style="width:3px;">Montos no afectos</th>
    ...
    <?php 
        $total_product_tax = 0;
        $ventas_exentas_total = 0;
        $ventas_gravadas_total = 0;
    ?>
    @php $i = 1 @endphp
    @foreach($lims_product_sale_data as $product_sale_data)
        <?php
            ...
            // Calcular valores para exentas y gravadas
            $precio_unitario = $product_sale_data->total / $product_sale_data->qty;
            $ventas_exentas = $product_sale_data->tax == 0 ? $product_sale_data->total : 0;
            $ventas_gravadas = $product_sale_data->tax > 0 ? $product_sale_data->total : 0;

            // Acumular totales
            $ventas_exentas_total += $ventas_exentas;
            $ventas_gravadas_total += $ventas_gravadas;
        ...
        <tr>
            ...
            <td style="width:5px; text-align: right;">
                {{ number_format((float)($precio_unitario), 2, '.', '') }}
            </td>
            <td style="width:3px;text-align: right;">{{ number_format((float)$ventas_exentas, 2, '.', ',') }}</td>
            <td style="width:3px;"></td>
           
            <td style="width:3px;"></td>
            <td style="width:3px; "></td>
            <td style="width:3px; text-align: right;">
                {{ number_format((float)$ventas_gravadas, 2, '.', ',') }}
            </td>
        </tr>
    @endforeach
</table>
...
<table class="default">
    <tr>
        <td colspan="8" style="text-align: right; font-weight: bold;">Total de gasto por cuenta del cliente:</td>
        <td style="width:3px; text-align: right;">
            {{ number_format((float)$ventas_exentas_total, 2, '.', ',') }}
        </td>
    </tr>
    <tr>
        <td colspan="8" style="text-align: right; font-weight: bold;">Total Ventas Gravadas:</td>
        <td style="width:3px; text-align: right;">
            {{ number_format((float)$ventas_gravadas_total, 2, '.', ',') }}
        </td>
    </tr>
    <tr>
        <td colspan="8" style="text-align: right; font-weight: bold;">Sub-total:</td>
        <td style="width:3px; text-align: right;">
            {{ number_format((float)($ventas_exentas_total + $ventas_gravadas_total), 2, '.', ',') }}
        </td>
    </tr>
</table>
            ...
            <div class="">
                <table style='min-height:10px; max-height:10px;'>
                    <tr>
                        ...
                        <td style='width:8%; text-align: right; padding-left: 20px; padding-right: 1px;'>
                            {{number_format((float)$lims_sale_data->grand_total-$ventas_exentas_total, 2, '.', ',')}}
                        </td>
                    </tr>
                </table>
            </div>
...
{{ number_format((float)$lims_sale_data->grand_total , 2, '.', ',') }}
        ...
        <div class="container-fluid" id="tabla1">  
            ...
            <div class="">
                <table style='min-height:10px; max-height:10px;'>
                    <tr>
                        <td style='width: 225%; text-align: left; font-size: 9px;'>Valor en letras: {{ $todo }}</td>
                    </tr>
                </table>
            </div>
        ...
        <div class="container-fluid" id="tabla1">  
            <div class="">
                <table style='min-height:10px; max-height:10px;'>
                    <tr>
                        <td style='width: 127%; font-size: 9px;'>Responsable por parte del Emisor: {{ $userName }}</td>
                        <td style='width: 100%; text-align: left; padding-left: 2px; padding-right: 40px;'>No. Documento: {{ $userNit }}</td>
                    </tr>
                </table>
            </div>
