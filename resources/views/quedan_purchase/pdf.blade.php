...
<html>
    ...
    <body>        
        ...
        <div id="receipt-data" class="contenedor">
            ...
            <div class="centered">
                @foreach($lims_quedan_data as $qr)
                    <div class="right">
                        {!! QrCode::size(60)->generate($qr->reference_no) !!}
                    </div> 
                @endforeach    
            </div>        
            ...
            <div class="centered">
                ...
                <p>
                    Sucursal que Emite el Quedan: {{$lims_warehouse_data->name}}
                    <br>
                    Dirección: {{$lims_warehouse_data->address}}
                    <br>
                    Telefono: {{$lims_warehouse_data->phone}}
                </p>
            </div>  

              
            
            @foreach ($lims_quedan_data as $v)
                @php
                    $meses = array("ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","AGOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE");
                    $fecha = \Carbon\Carbon::parse($v->created_at);
                    $mlibro = $meses[($fecha->format('n')) - 1];
                @endphp
                @php $ylibro = \Carbon\Carbon::parse($v->created_at)->format('Y')  @endphp
                ...
                <div style="max-width:560px;margin:0 auto">
                    ...
                    <div class="">
                        <table  style='min-height:10px; max-height:10px;'>
                            <tr>
                                <td style='width: 60%'>Número de Quedan: <font color="#FF0000"> {{ $v->id }} </font></td>
                                <td style='width: 87%'>Fecha de Emisión: {{ \Carbon\Carbon::parse($v->created_at)->format('d/m/Y') }} </td>
                    ...
                    <div class="">
                        <table   style='min-height:10px; max-height:10px;'>
                            <tr>
                                <td style='width: 60%'>a Favor de: {{ $v->name }}</td>
                                <td style='width: 87%'>Fecha estimada de Pago: {{ \Carbon\Carbon::parse($v->due_date)->format('d/m/Y') }} </td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="">
                        <table  style='min-height:10px; max-height:10px;'>
                            <tr>
                                <td style='width: 34%; height: 15px;'>por un Valor de: ${{ number_format($v->total,2) }}</td>
            ...
            <div class="e1" style="padding-top: 8px;">
                <table>
                    ...
                    @php $stotal=0 @endphp
                    @foreach ($lims_detalle_quedan_data as $d)
                        @php $stotal += $d->grand_total @endphp
                        <tr>
                            <td style="width:15px; text-align: left;">{{ \Carbon\Carbon::parse($d->created_at)->format('d/m/Y') }}</td>
                            <td style="width:15px; text-align: left;">{{ $d->invoice }}</td>
                            <td style="width:15px; text-align: left;">USD</td>
                            <td id="totales" class="text-right" colspan="1" style="width:15px;">{{ number_format($d->grand_total, 2) }}</td>
                        </tr>
                    @endforeach

                    @foreach ($lims_detalle_quedan_nc_data as $d)
                        @php $stotal -= $d->grand_total @endphp
                        <tr>
                            <td style="width:15px; text-align: left;">{{ \Carbon\Carbon::parse($d->created_at)->format('d/m/yy') }}</td>
                            <td style="width:15px; text-align: left;">NC - {{ $d->invoice }}</td>
                            <td style="width:15px; text-align: left;">USD</td>
                            <td id="totales" class="text-right" colspan="1" style="width:15px;">-{{ number_format($d->grand_total, 2) }}</td>
                        </tr>
                    @endforeach
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th style="width:15px;"></th>
                            <th id="totales" class="text-right" colspan="1">${{ number_format($stotal,2) }}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <div id="receipt-data" class="contenedor">
            ...
            @foreach ($lims_quedan_data as $v)
                @php
                    ...
                    $mlibro = $meses[($fecha->format('n')) - 1];
                @endphp
                @php $ylibro = \Carbon\Carbon::parse($v->date_quedan)->format('y')  @endphp
